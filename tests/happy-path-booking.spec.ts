...
test.describe('Appointment Widget', () => {
  ...
  test('Complete booking with valid customer details',{tag: ['@smoke', '@functional']}, async ({ page }) => {
    ...
    const firstService = page.getByRole('link', { name: 'Update Personal Account  60' });
    ...
    const locationButton = page.getByRole('button', { name: 'McKinney 2093 N. Central' });
    ...
    await page.getByLabel(datePickerLabel).getByRole('link', { name: dayNum.toString(), exact: true }).click();

    let selectedTime = '';
    await expect(async () => { // this await block retried if it fails. (the feature has a loader which sometimes takes a while to load )
        ...
        .filter({ hasNot: page.locator('.disabled, [disabled], .grayed-out') }) // Exclude typical disabled classes
        .first();
    ...
    await page.getByRole('textbox', { name: 'Email' }).fill('ronha.smith@example.com');
    await page.getByRole('textbox', { name: 'Phone No.' }).fill('512-555-9876');
    ...
    await expect(async () => {
      await expect(page.getByRole('heading', { name: /.*appointment has been scheduled/ }))
    ...
    await expect(page.locator(`text=${expectedConfirmationText}`)).toBeVisible();
